...
void HttpServer::onMessage(const TcpConnectionPtr& coon,Buffer* buf,Timestamp recvtime)//TcpConnection中的buffer,接受的HTTP请求存在这里
{
   std::unique_ptr<HttpAnalyse>  yser(new HttpAnalyse());
   
   if(!yser->parseRequest(buf,recvtime))//消息头并储存结果
   {
...
void HttpServer::OnRequest(const TcpConnectionPtr& conn,const HttpRequest& req)
{
   ...
   (req.Getversion()==HttpRequest::VERSION::HTTP10&&connection!="Keep-alive"));//只接受HTTP1.1
   HttpRespne respone(close);//
   httpCallback_(req,&respone);//消息设置,设置状态和设置回复
   Buffer buf;
   respone.appendToBuffer(&buf);//更具设置情况 组装HTTP恢复报文
