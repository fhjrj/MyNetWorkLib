...
#include <string>
    ...
    char* DataSegment::SerializeToCharArray()
    {
        if(final_packet_!=nullptr)
        {
         ...
         {
            final_packet_= reinterpret_cast<char*>(calloc(MAX_PACKET_SIZE, sizeof(char)));//开辟数据包
    ...
    void  DataSegment::DeserializeToDataSegment(unsigned char *data_segment,int length)//接受对面传输的数据，网络字节序 大段 所有要转化为小端
    {
